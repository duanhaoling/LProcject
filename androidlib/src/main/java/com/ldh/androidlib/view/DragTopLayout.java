...
public class DragTopLayout extends FrameLayout {
    ...
    private boolean captureTop = false;
    ...
    private onTopViewDown onTopViewDownListener;
    ...
    // add by Tim
    public interface onTopViewDown {
        void onTopViewDown();
    }
    ...
    private void init(AttributeSet attrs) {
        ...
        setCollapseOffset(collapseOffset);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (needResize && dragContentView.getY() != collapseOffset) {
            int height = dragContentView.getMeasuredHeight();
            height -= topViewHeight;
            dragContentView.measure(MeasureSpec.makeMeasureSpec(dragContentView.getMeasuredWidth(),
                    MeasureSpec.EXACTLY),
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        dragContentView.layout(left, contentTopTemp, right,
                contentTopTemp + dragContentView.getMeasuredHeight());
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //整体控件高度增大的时候，dragContentView的高度不正确导致下方出现空白
        if (h - oldh > 50) {
            //因为有ListView会出现高度计算不正确，这里重新测量高度
            int height = h - topViewHeight;
            if (panelState == PanelState.COLLAPSED) {  //顶部View收起时
                height = h;
            }
            dragContentView.measure(MeasureSpec.makeMeasureSpec(dragContentView.getMeasuredWidth(),
                    MeasureSpec.EXACTLY),
    ...
    private ViewDragHelper.Callback callback = new ViewDragHelper.Callback() {
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            if (yvel > 0 || contentTop > topViewHeight) {
                ...
                if (null != onTopViewDownListener) {
                    onTopViewDownListener.onTopViewDown();
                }
            } else {
     ...
     * add by tim
     *
     * @param needResize 是否要重新计算contentView的高度(-topView的高度)
     */
     ...
     * 设置当topview滑下来时的监听
     */
    public void setOnTopViewDownListener(onTopViewDown onTopViewDownListener) {
        this.onTopViewDownListener = onTopViewDownListener;
    }
