...
public abstract class BaseNiceDialog extends DialogFragment {
    ...
    private static final String LOCAL_STYLE = "top_style";
    ...
    private boolean isLocalStyle = false; //添加APP共同风格

    public abstract int initLayoutId();

    public abstract void convertView(DialogViewHolder holder, BaseNiceDialog dialog);
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            isLocalStyle = savedInstanceState.getBoolean(LOCAL_STYLE);
        }
    }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (isLocalStyle) {
            ...
            LinearLayout ll = (LinearLayout) view.findViewById(R.id.dialog_content);
            ll.addView(contentView);
        } else {
            view = inflater.inflate(layoutId, container, false);
        }
        convertView(DialogViewHolder.create(view, this), this);
        return view;
    }
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        outState.putInt(HEIGHT, width);
        outState.putFloat(DIM, dimAmount);
        outState.putBoolean(BOTTOM, showBottom);
        outState.putBoolean(CANCEL, outCancel);
        outState.putInt(ANIM, animStyle);
        outState.putInt(LAYOUT, layoutId);
        outState.putBoolean(LOCAL_STYLE, isLocalStyle);
    }
    ...
    private void initParams() {
        ...
        if (window != null) {
            ...
            //设置dialog宽度
            if (width == 0) {
                lp.width = DialogUtil.getScreenWidth(getContext()) - 2 * DialogUtil.dp2px(getContext(), margin);
            } else {
                lp.width = DialogUtil.dp2px(getContext(), width);
            }
            //设置dialog高度
            if (height == 0) {
                lp.height = WindowManager.LayoutParams.WRAP_CONTENT;
            } else {
                lp.height = DialogUtil.dp2px(getContext(), height);
            }
    ...
    public BaseNiceDialog isLocalStyle(boolean isLocalStyle) {
